...
it('can force delete', function () {
    ...
    expect(Product::withTrashed()->where('uuid', $product->uuid)->exists())->toBeTrue();

    $product->forceDelete();

    expect(Product::withTrashed()->where('uuid', $product->uuid)->exists())->toBeFalse();
});
...
it('can restore', function () {
    ...
    expect(Product::withTrashed()->where('uuid', $product->uuid)->first()->deleted_at)->not->toBeNull();

    $product->restore();

    expect(Product::withTrashed()->where('uuid', $product->uuid)->first()->deleted_at)->toBeNull();
});
